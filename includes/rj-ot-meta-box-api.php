<?php 
if ( ! class_exists( 'RJ_OT_User_Meta_Box' ) ) {

  class RJ_OT_User_Meta_Box {
    ...
    function __construct( $meta_box ) {
      ...
      //add_action( 'save_post', array( $this, 'save_meta_box' ), 1, 2 );
		...
		add_action( 'personal_options_update', array( $this, 'save_meta_box' ), 1, 2  );
		add_action( 'edit_user_profile_update',  array( $this, 'save_meta_box' ) , 1, 2 );
     ...
     * @since     1.0
     */
    function add_meta_boxes($user) {
     rj_ot_admin_scriptss();
		rj_ot_admin_styless();
		foreach ( (array) $this->meta_box['id'] as $page ) {
			$this->build_user_meta_box($user ,$this->meta_box['fields'] ,$this->meta_box['id'],$this->meta_box['title']);
		 }
    }
    ...
    function save_meta_box( $post_id ) {
		...
		$post_id = ($post_id) ? $post_id : $_REQUEST['user_id'];
    ...
    // check permissions 
      if ( isset( $_POST['post_type'] ) && 'page' == $_POST['post_type'] ) {
        ...
        if ( ! current_user_can( 'edit_user', $post_id ) )
          return $post_id;
      }
        */
        if ( ! current_user_can( 'edit_user', $post_id ) )
      ...
      foreach ( $this->meta_box['fields'] as $field ) {
        ...
        $old = get_user_meta( $post_id, $field['id'], true );
        $new = '';
        ...
        if ( isset( $new ) && $new !== $old ) {
          ...
          update_user_meta( $post_id, $field['id'], $new );
        } else if ( '' == $new && $old ) {
         // delete_user_meta( $post_id, $field['id'], $old );
 ...
 * @since    2.0
 */
if ( ! function_exists( 'rj_ot_user_register_meta_box' ) ) {

  function rj_ot_user_register_meta_box( $args ) {
    if ( ! $args )
      return;
      
    $ot_meta_box = new RJ_OT_User_Meta_Box( $args );
  }
