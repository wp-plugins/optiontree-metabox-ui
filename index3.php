...
if ( ! function_exists( 'ot_sections_view' ) ) {
  ...
  function ot_sections_view( $name, $key, $section = array() ) {
	...
	$disabled_post_types = array('page','attachment','revision','nav_menu_item','option-tree');
	 ...
	 $post_type_template = "<select  name='" . esc_attr( $name ) . "[" . esc_attr( $key ) . "][pages]"."' value=\"\" class=\"option-tree-ui-select\"><option value=\"page\">Page</option>";
	  foreach ( $post_types as $keya => $value ) {
		  if(!in_array($value,$disabled_post_types)){
	 if($section['pages']==$value){
  ...
  $page_template = "<select  name='" . esc_attr( $name ) . "[" . esc_attr( $key ) . "][template]"."' value=\"\" class=\"option-tree-ui-select\"><option value=\"default\">Default</option>";
 foreach ( $templates as $template_name => $template_filename ) {
	 if($section['template']==$template_filename){
  ...
  $all_pages_template = "<select  name='" . esc_attr( $name ) . "[" . esc_attr( $key ) . "][page]"."' value=\"\" class=\"option-tree-ui-select\"><option value=\"default\">Default</option>";
 foreach ( $all_pages as $sin_page ) {
	 if($section['page']==$sin_page->ID){
