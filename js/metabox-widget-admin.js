      ...
      if ( this.processing === false ) {
        ...
        $.ajax({
          ...
          complete: function( data ) {
            ...
            var uploadelem = list.children().last().find('.ot_upload_media').prev('input');
             var uploadelem_id =  uploadelem.attr('id');
              uploadelem_id = uploadelem_id.replace(/\[/g,'_');
              uploadelem_id = uploadelem_id.replace(/\]/g,'_');
              uploadelem.attr('id',uploadelem_id);
            if ( type != 'the_contextual_help' ) {
              OT_UI.update_ids(list);
            }
    ...
    init_upload: function() {
      $(document).on('click', '.ot_upload_media', function() {
        ...
        var field_id            = $(this).parent('.option-tree-ui-upload-parent').find('input').attr('id'),
          ...
          var uploadelem = $(this).parent('.option-tree-ui-upload-parent').find('input');
             var field_id =  uploadelem.attr('id');
              field_id = field_id.replace(/\[/g,'_');
              field_id = field_id.replace(/\]/g,'_');
              uploadelem.attr('id',field_id);
